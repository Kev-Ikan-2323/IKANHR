...
export var DB = {
  ...
  async insert(tableName, record) {
    ...
    // Remove empty-string dates so PostgreSQL doesn't fail on DATE fields
    Object.keys(row).forEach(function(k) {
      if (row[k] === '') {
        // Keep empty strings for text columns, but nullify for known date/numeric cols
        ...
        if (isDateCol || isNumCol) row[k] = null
      }
    })
  ...
  async update(tableName, id, changes) {
    ...
    // Nullify empty date/numeric strings
    Object.keys(row).forEach(function(k) {
      if (row[k] === '') {
        var isDateCol = k.endsWith('_at') || k.endsWith('_date') || k === 'date'
        var isNumCol  = k.endsWith('_score') || k.endsWith('_days') || k === 'year' || k === 'weight' || k === 'level'
