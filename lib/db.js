...
export var DB = {
  ...
  async query(tableName, filters) {
    ...
    if (filters) {
      Object.keys(filters).forEach(function(k) {
        ...
        var col = toSnake(k)
        // FK/numeric columns must use eq; text columns use ilike for case-insensitive match
        var isNumericCol = col.endsWith('_score') || col.endsWith('_days') || col === 'year' || col === 'month' || col === 'weight' || col === 'level'
        var isFkCol = col.endsWith('_id') && col !== 'id'
        if (typeof val === 'string' && !isNumericCol && !isFkCol) {
          q = q.ilike(col, val)
        } else {
          q = q.eq(col, val)
        }
  ...
  async hardDelete(tableName, id) {
    ...
    if (error) throw new Error('DB.hardDelete [' + table + ']: ' + error.message)
    return { id }
  },
